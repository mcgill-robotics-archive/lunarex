...
public class Client extends Thread {
	...
	public void run() {
		try {
			DataOperaion operation = new DataOperaion(); // Operations on data,
															// sent from client
															// to server
			...
			while (true) {
				try {
					...
					ObjectInputStream in = new ObjectInputStream(inFromServer); // Read
																				// and
																				// write
																				// object
					DataStream stream = null; // Status of the robot, sent from
												// server to client
					System.out.print("Quit?");
					if (data.hasNext()) {
						operation.q = data.nextBoolean();
					} // Close connection or not
					if (!operation.quit()) // If not quiting
					{
						System.out
								.print("Accelerate or decelerate? (true/false): ");
						if (data.hasNext()) {
					...
					// for(int i = 0; i < operation.getCycle(); i++) //For-loop
					// retained.
					// {
