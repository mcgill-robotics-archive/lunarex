...
def handle_service(req):
    ...
    kinect_distance = np.array(createMap(), np.int32)
    print "Kinect distance: " + str(kinect_distance)
    return kinectSafeDistanceResponse(kinect_distance)
    #return KinectDataResponse(kinect_matrix.shape[0], kinect_matrix.shape[1], np.reshape(kinect_matrix, -1))

def service_server():
    s = rospy.Service('kinect_service', kinectSafeDistance, handle_service)
...
#		sobel_filter[y][x] = int( variance)
    ...
    height_difference_threshold = 12 #cm
    for y in range(1, len(sobel_filter )-1):
	for x in range(1,len(sobel_filter[0])-1):
		if (projection[y][x][1] == 0):
			continue;	
    ...
    #not ignore everything more than 3 m  away
    #for y in range(0, 11):
	#	for x in range(1,len(sobel_filter[0])-1):
	#		sobel_filter[y][x] = -1		
    ...
    for i in range(1,40):
		y = 40 - i
		for x in range(9, 29):   # assuming robot is 1 meter wide 
			if (sobel_filter[i][x] == 100):
				safe_distance = y

	# return safe_distance # as a service ... 
    return safe_distance # in decimeters
