...
# current position == last demanded goal
...
current_heading = 0
global OBSTACLE_BOUNDARY_LOCATION
OBSTACLE_BOUNDARY_LOCATION = 2#???????
global subgoals
subgoals = []
global list_of_angles_to_scan
list_of_angles_to_scan  = [0, 15, 30, 45, -15, -30, -45]
# subgoals_from start_side_to_mining_side will store all the stored goals in order to go from the start zone
# to the mining zone. 
# to go back to the start zone to dump, use the goals sstarting from the end
#elements :: [x, y, theta]

# populate the list by where the search fot a path will start
# this should be about 1 m in front of the mining area and at the center of the arena
subgoals.append([ARENA_WIDTH/2, OBSTACLE_BOUNDARY_LOCATION-1, 90 ]) # [x, y, theta]
global mining_area_goal
mining_area_goal = [ARENA_WIDTH/2, MINING_BOUNDARY_LOCATION+2, 90]
...
def goTo(x,y,theta):
	current_x =  x					###seb modification
	current_y =  y					###seb modification
	current_heading = theta			###seb modification
	...
	if(theta!= 'non_specified_angle'):			###seb modification
...
def straightAheadDistanceSafe():
	return 3 
	rospy.wait_for_service('kinect_service')
	request = rospy.ServiceProxy('kinect_service', kinectSafeDistance)
	kinect_distance = self.request(0)		#Send a request (containing any int) and get response
	kinect_distance_in_meters = kinect_distance / 10.0
	return kinect_distance_in_meters

def first_time_go_to_mining_area():
	# go to mining area and initialize the subgoals list 
	# that will be used to come back and to to the mining area the second time
	# goes to mining_area_goal

	#go the the preset first element of the subgoal list
	goTo(subgoals[0][0], subgoals[0][1], 90)
	max_counter = 0
	while(max_counter<6 and current_y<MINING_BOUNDARY_LOCATION+0.5):
		max_counter+=1
		moved = False
		for angle in list_of_angles_to_scan:
			if(not moved):
				#goTo(subgoals[0][0], subgoals[0][1], 90+angle)
				HectorAngle = coord.arenaAngle2hectorAngle(theta+angle, LR_corner, RR_corner, RF_corner, LF_corner)
				spinToHectorAngle(HectorAngle)
				if (straightAheadDistanceSafe()>=2.5):
					goTo(current_x + 1.5* math.sin(angle*math.pi/180), current_y + math.cos(angle*math.pi/180) *1.5 , 90)
					moved = True
					subgoal.append([current_x,current_y, 90])
		if(moved == False):
			goTo(current_x, current_y+1, 'non_specified_angle')


def goToMiningArea():
	if(len(subgoals<2)):
		first_time_go_to_mining_area()
		return
	if(current_y>MINING_BOUNDARY_LOCATION):
		return  #already there!
	for goal in subgoals:
		if(goal[1]> current_y):
			goTo(goal[0], goal[1], 'non_specified_angle')


def goToDumpingArea():
	if(current_y< OBSTACLE_BOUNDARY_LOCATION):
		return  #already there!
	for i in range(len(subgoals)):
		goal = subgoals[len(subgoals)-1-i]
		if(goal[1]<current_y):
			goTo(goal[0], goal[1], 'non_specified_angle')
